...
if page == "Home":
    ...
    fig1 = px.line(df, x=df['Date'], y=to_plot, title='AM, Noon, and PM Readings')
    ...
    n_list = df['Glucose Noon'].replace(to_replace='None', value=np.nan).dropna().astype(int)
    ...
    with c2:
        ...
        st.write(f'**Average:** :red[{math.trunc(n_list.sum() / (len(df) - 1))}*]')
    ...
    st.dataframe(df.dropna(how='all'), width=800)
    ...
    def add_new_reading():
        if submit:
            ...
            new_df = pd.DataFrame({'Date': date, 'Time (AM)': time, 'Glucose': num}, index=[0])
            st.session_state['def_df'] = pd.concat([st.session_state['def_df'], new_df], axis=0)
            st.dataframe(st.session_state['def_df'].replace(np.nan, 'None'), width=800)            
           
           
          
        
    glucose_list = df2['Glucose'].replace(to_replace='None', value=np.nan).dropna().astype(int)
    g_sum= glucose_list.sum()
    num_of_v = len(df2) - 1
    
    fig1 = px.line(df2, x=df2['Date'], y=df2['Glucose'], title="Glucose levels from June to Sept")
    st.plotly_chart(fig1)
    ...
    with st.form('my_form'):
        ...
        submit = st.form_submit_button(label="Submit", on_click=add_new_reading)
